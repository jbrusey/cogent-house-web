...
def _locCalibrateReadings(session, theQuery):
    ...
    for reading in theQuery:
        ...
        if not theSensor:
            theSensor = (
                session.query(sensor.Sensor)
                .filter_by(nodeId=reading.nodeId, sensorTypeId=reading.typeId)
                .first()
            )
...
def _query_by_location_and_type_with_join_target(
    session,
    loc_id,
    reading_type,
    start_time,
    ...
    if reading_type in reading_limits and filter_values:
        theQuery = (
            session.query(Reading, target.c.Reading_value)
            .filter(
                and_(
...
def _query_by_type_location(
    session, reading_type, start_time, end_time, filter_values=True, calib=True
):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        theQuery = session.query(Reading).filter(
            and_(
                Reading.time >= start_time,
                Reading.time < end_time,
                Reading.typeId == rtype_id,
...
def _query_by_location_and_type(
    session, loc_id, reading_type, start_time, end_time, filter_values=True, calib=True
):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        theQuery = (
...
def _query_by_type_with_join_target(
    session, reading_type, start_time, end_time, target, filter_values=True
):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        return session.query(
            Reading.nodeId, Reading.time, Reading.value, target.c.Reading_value
        ).filter(
            and_(
                Reading.time >= start_time,
                Reading.time < end_time,
...
def get_locations_by_house(session, house_id, include_external=True):
    ...
    rows = (
        ...
        .filter(Location.houseId == house_id)
        .join(House, Room)
...
def get_data_by_location_and_type(
    session,
    loc_id,
    reading_type,
    ...
    if (
        ...
        print(
            "Cleaning is being applied to reading type %s, this is not generally wanted. Check your code!"
            % reading_type,
            file=sys.stderr,
        )
    ...
    if with_deltas:
        ...
        rows = _query_by_location_and_type_with_join_target(
            session,
            loc_id,
            reading_type,
            start_time,
        ...
        rows = _query_by_location_and_type(
            session, loc_id, reading_type, start_time, end_time, filter_values=False
        )
...
def get_location_types(
    ...
    theQuery = (
        session.query(distinct(Reading.typeId))
        .filter(
            and_(
                Reading.time >= start_time,
                Reading.time < end_time,
...
def get_data_by_location_and_type_with_battery(
    session,
    loc_id,
    reading_type,
    start_time=datetime.datetime.fromtimestamp(0),
...
def get_data_by_type_location(
    ...
    if with_deltas:
        delta_rows = (
            _query_by_type(
                session, "d_" + reading_type, start_time, end_time, filter_values=False
            )
            .with_labels()
            .subquery()
        )
        rows = _query_by_type_with_join_target(
            session, reading_type, start_time, end_time, delta_rows, filter_values=False
        )
        rows = rows.join((delta_rows, Reading.time == delta_rows.c.Reading_time))
    else:
        rows = _query_by_type_location(
            session, reading_type, start_time, end_time, filter_values=False
        )
...
def clean_data(session, data, reading_type=None):
    ...
    if isinstance(data, list) or isinstance(data, ReadingList):
        ...
        for key, value in data.items():
            ...
            if len(cd) > 0:
                cleaned[key] = cd
        return cleaned
    elif isinstance(data, sqlalchemy.orm.query.Query):
...
def _clean_data(session, data, reading_type=None):
    ...
    if reading_type is not None:
        print(
            "[db_access._clean_data] Use of the reading_type parameter is deprecated.",
            file=sys.stderr,
        )

    if isinstance(data, ReadingList) and reading_type is None:
        reading_type = data.get_meta_data("reading_type")

    if reading_type is not None and reading_type in reading_limits:
...
def get_yield(
    ...
    expected_rows = float(days * 288.0)

    return 100.0
    row_count = _query_by_node_and_type(
        session, node_id, reading_type, start_time, end_time
    ).count()

    return (row_count * 100.0) / expected_rows


def get_yield_location(
    session,
    loc_id,
    reading_type,
    start_time=datetime.datetime.fromtimestamp(0),
    ...
    rows = _query_by_location_and_type(
        session, loc_id, reading_type, start_time, end_time
    )
...
def get_yield_by_nodes_and_date(
    session,
    hnum,
    reading_type,
    start_time=datetime.datetime.fromtimestamp(0),
    ...
    for room, lid in locations.items():
        ...
        while start < end_time:
            rows = _query_by_location_and_type(
                session, lid, reading_type, start, plus_one
            )
            row_count = len(list(rows))
...
def node_has_co2(session, node_id):
    # Fuck It
    # co2 is type 8
    qry = session.query(Reading).filter_by(nodeId=node_id, typeId=8)
    return qry.first()
