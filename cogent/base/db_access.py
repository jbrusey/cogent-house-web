...
def fetch_calib(filename, dtype):
    calib_file = csv.reader(
        open(os.path.join(os.path.dirname(__file__), "Calibration", filename), "r"),
        delimiter=",",
    )
...
def get_calibration_db(session, node_id, reading_type):
    ...
    row = (
        ...
        .filter(and_(Sensor.nodeId == node_id, Sensor.sensorTypeId == rtype_id))
        .first()
    )
    if row is None:
        return (1.0, 0.0)
...
def calibrate(session, data, node_id=None, reading_type=None, cal_func=get_calibration):
    if node_id is not None or reading_type is not None:
        print(
            "[db_access.calibrate] Use of the node_id and reading_type parameters is deprecated.",
            file=sys.stderr,
        )

    if reading_type is None:
        if isinstance(data, ReadingList):
...
def _query_by_node_and_type_with_join_target(
    session, node_id, reading_type, start_time, end_time, target, filter_values=True
):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        return session.query(
            Reading.time, Reading.value, target.c.Reading_value
        ).filter(
            and_(
...
def _query_by_node_and_type(
    session, node_id, reading_type, start_time, end_time, filter_values=True
):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        return session.query(Reading.time, Reading.value).filter(
            and_(
                Reading.time >= start_time,
                Reading.time < end_time,
                Reading.nodeId == node_id,
...
def _query_by_type(session, reading_type, start_time, end_time, filter_values=True):
    rtype_id = reading_types.index(reading_type)
    if reading_type in reading_limits and filter_values:
        return session.query(Reading.nodeId, Reading.time, Reading.value).filter(
            and_(
                Reading.time >= start_time,
                Reading.time < end_time,
                Reading.typeId == rtype_id,
...
def get_data_by_node_and_type(
    ...
    if with_deltas:
        ...
        rows = _query_by_node_and_type_with_join_target(
            session,
            node_id,
            reading_type,
            start_time,
        ...
        rows = _query_by_node_and_type(
            session, node_id, reading_type, start_time, end_time, filter_values=False
        )
