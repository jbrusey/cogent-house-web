...
class RRDStore(object):
    ...
    def __init__(self, nodeId, typeId, locationId, startTime=None):
        ...
        if os.path.exists(rrdfd):
            ...
            # (288 samples per day)
            archives.append(
                rrd.RRA(cf="AVERAGE", xff=0.5, steps=1, rows=8064)
            )  # 5 Minute Samples for 1 month
            archives.append(
                rrd.RRA(cf="AVERAGE", xff=0.5, steps=12, rows=2016)
            )  # Every Hour for 3 months
            archives.append(rrd.RRA(cf="MIN", xff=0.5, steps=12, rows=2016))
            archives.append(rrd.RRA(cf="MAX", xff=0.5, steps=12, rows=2016))
            archives.append(
                rrd.RRA(cf="AVERAGE", xff=0.5, steps=288, rows=365)
