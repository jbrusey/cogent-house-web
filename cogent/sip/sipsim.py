...
PhenomTuple = namedtuple("PhenomTuple", "dt, sp, ev, ls, lt, s, t, xn, x, v, dashed")
...
class Phenom(object):
    ...
    def __init__(self, init_x=0.0, init_v=0.0, accel_var=0.0):
        self.init_x = init_x
        self.init_v = init_v
    ...
    def __iter__(self):
        while True:
            yield NULL_PHENOM._replace(x=self.init_x, v=self.init_v)
...
class PhenomCsvFile(object):
    ...
    def __init__(self, filename=None, delimiter=",", column=0):
...
class Noise(object):
    ...
    def __init__(self, src=None, var=0.0):
        self.phen = src
        self.var = var

    def __iter__(self):
        for pt1 in self.phen:
            yield pt1._replace(xn=pt1.x + random.gauss(0, self.var))
...
class Dewma(object):
    ...
    def __init__(self, src=None, alpha=0.0, beta=0.0):
        self.src = src
        self.alpha = alpha
        self.beta = beta
        self.si = None
...
class Event(object):
    ...
    def __init__(self, src=None, threshold=0.5):
        self.src = src
    ...
    def __iter__(self):
        for ptup in self.src:
            if self.last is None or abs(self.last[0] - ptup.s) > self.threshold:
                yield ptup._replace(ev=True, ls=ptup.s, lt=ptup.t)
                self.last = (ptup.s + ptup.t, ptup.t)
            else:
                yield ptup._replace(ev=False, ls=self.last[0], lt=self.last[1])
                (s1, t1) = self.last
                self.last = (s1 + t1, t1)
...
class SipPhenom(object):
    ...
    def __iter__(self):
        ...
        for date, value, delta, seq in self.src:
            if first:
                yield NULL_PHENOM._replace(
                    ev=True, ls=value, lt=delta, s=value, t=delta, dt=date, dashed=False
                )
                ...
                while date - last_dt > self.interval + self.duplicate_interval:
                    ...
                    yield NULL_PHENOM._replace(
                        ev=False, ls=last_s, lt=last_t, dt=last_dt, dashed=dashed
                    )
...
class Spline(object):
    ...
    def __iter__(self):
        for t in range(1, self.steps + 1):
            yield self.calc_poly(t * 1.0 / (self.steps))
...
class QuadStartSpline(Spline):
    ...
    def __init__(self, a, b, c, steps):
        c *= steps
        Spline.__init__(self, [b - a - c, c, a], steps)
...
class QuadEndSpline(Spline):
    ...
    def __init__(self, a, b, c, steps):
        c *= steps
        Spline.__init__(self, [a - b + c, -2 * a + 2 * b - c, a], steps)
        ...
        for ptup in self.src:
            ...
            if ptup.ev:
                if self.last is not None:
                    ...
                    spline = CubicSpline(
                        self.last[0], self.last[1], ptup.s, ptup.t, len(self.stack)
                    )
...
class PartSplineReconstruct(object):
    ...
    def __init__(self, src=None, threshold=None):
        self.src = src
        self.stack = []
        self.last = None
        self.threshold = threshold
        ...
        for ptup in self.src:
            ...
            if ptup.ev:
                if ptup.dashed:
                    ...
                    gradient = (ptup.ls - start_x) * 1.0 / len(self.stack)
                    ...
                    start_spline = QuadStartSpline(
                        self.last[0], xhat_1, self.last[1], len(self.stack) - 1
                    )
                    end_spline = QuadEndSpline(xhat_1, ptup.s, ptup.t, 1)
...
class QuarticSpline(Spline):
    def __init__(self, a, b, c, d, e, s, steps):
        assert s > 0 and s < 1
        b *= steps
        d *= steps
        s2 = s * s
        ...
        Spline.__init__(
            self,
            [A, -2 * A + 2 * a + b - 2 * c + d, A - 3 * a - 2 * b + 3 * c - d, b, a],
            steps,
        )
        ...
        for ptup in self.src:
            ...
            if ptup.ev:
                if self.last is not None:
                    ...
                    spline = CubicSpline(
                        self.last[0], self.last[1], ptup.s, ptup.t, steps
                    )
                    ...
                    if e is not None:
                        spline = QuarticSpline(
                            self.last[0], self.last[1], ptup.s, ptup.t, e, xk1, steps
                        )
                    for v in spline:
                        yield self.stack.pop(0)._replace(sp=v)
