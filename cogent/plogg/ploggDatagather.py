...
class PloggCollector(object):
    ...
    def connect(self, thePort):
        ...
        try:
            ...
            return False
        self.log.debug(
            "Port {0} opened at baud({1})".format(p.portstr, p.getBaudrate())
        )
    ...
    def loop(self):
        ...
        while True:
            try:
                ...
                log.info(
                    "Mainloop complete in {0} Sleeping for {1}".format(
                        loopTime, sleepTime
                    )
                )
                if sleepTime > 0:
    ...
    def saveData(self, nodeId, values):
        ...
        # Fetch Sensor Types
        wattSensor = (
            session.query(models.SensorType).filter_by(name="Plogg Watts").first()
        )
        ...
        currentSensor = (
            session.query(models.SensorType).filter_by(name="Plogg Current").first()
        )
        ...
        # And add a nodeState
        theNodeState = models.NodeState(
            time=sampleTime, nodeId=theNode.id, parent=theNode.id, localtime=sampleTime
        )
    ...
    def ploggDebug(self, text):
        ...
        if len(ha) == 57:
            ...
            log.debug(
                "Plogg Uptime {0}".format(ploggup)
            )  # strange one ignore for now, not really improtant
            ...
            equipup = uptimeProcess(
                (float(int(str(ha[51] + ha[52] + ha[53] + ha[54]), 16)) / 100)
            )
...
if __name__ == "__main__":
    ...
    group = OptionGroup(
        parser, "Sets Sampling Rate:", "Set the sampling rate of the plogg sensors"
    )
    ...
    group.add_option(
        ...
        "--rate",
        dest="srate",
        help="Set plogg sampling rate in seconds",
        default=60,
        type="int",
    )
